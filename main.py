...
def train(model, loader, optimizer, device, p_e, p_f, p_n, lambda_jsd, use_edge_drop=False, use_feature_mask=False, use_node_drop=False):
    model.train()
    total_loss = 0
...
if __name__ == "__main__":
    ...
    use_node_drop = False    # Enable node dropping augmentation
    ...
    p_n = 0.1  # Node drop probability (only used when use_node_drop=True)
    ...
    for fold_idx, (train_loader, val_loader) in enumerate(folds):
        ...
        for epoch in range(1, epochs + 1):
            train_loss = train(model, train_loader, optimizer, device, p_e, p_f, p_n, lambda_jsd, use_edge_drop, use_feature_mask, use_node_drop)
